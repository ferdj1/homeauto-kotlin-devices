...
cleanup() {
    ...
    nmcli connection down homeLockSystems-Init535
...
then
    ...
    if test -e credentials.txt ; then
        ...
        nmcli connection show homeLockSystems-Init535 > /dev/null
        if [ $? != 0 ]; then
            nmcli device wifi hotspot con-name homeLockSystems-Init535 ssid homeLockSystems-Init535 band bg password hltvws43hd535
        else
            nmcli connection up homeLockSystems-Init535
        fi
